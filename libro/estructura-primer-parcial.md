...
```mermaid
flowchart LR
  A["Charcot: histeria traumatica"] --> B["Breuer/Freud: Anna O. e hipnosis"]
  B --> C["Metodo catartico: abreaccion"]
  C --> D["Freud solo: Psicoterapia de la histeria"]
  D --> E["Presion en la frente"]
  E --> F["Resistencia"]
  F --> G["Defensa / represion / metodo analitico"]
```
...
```mermaid
flowchart LR
  A["Ocasionamiento / vivencia"] --> B["Referencia simbolica o linguistica"]
  B --> C["Sintoma corporal"]
  C --> D["Texto a descifrar"]
```
...
```mermaid
flowchart LR
  A["Estimulo / vivencia"] --> B["Suma de excitacion / monto de afecto"]
  B --> C["Tramitacion normal"]
  C --> D["Via motriz: llorar, golpear, actuar"]
  C --> E["Palabra"]
  C --> F["Procesamiento asociativo / representacion contrastante"]
  D --> G["Descarga / abreaccion"]
  E --> G
  F --> G
```
...
```mermaid
flowchart TD
  A["Sintoma"] --> B["Cadena asociativa 1"]
  A --> C["Cadena asociativa 2"]
  A --> D["Cadena asociativa 3"]
  B --> E["Punto nodal"]
  C --> E
  D --> E
  E --> F["Nucleo patogeno"]
...
```mermaid
flowchart TD
  A["Dolor en la pierna"] --> B["Serie del padre"]
  A --> C["Serie de la hermana"]
  A --> D["Serie del caminar / detenerse"]
  A --> E["Serie de la soledad / independencia"]
  B --> F["Punto nodal: conflicto psiquico"]
  C --> F
  D --> F
  E --> F
```
...
```mermaid
flowchart LR
  A["Estimulos internos / externos"] --> B["Polo perceptivo"]
  B --> C["Sistemas de huellas mnemicas"]
  C --> D["Polo motor"]
  D --> E["Descarga / inervacion"]
```
...
```mermaid
flowchart LR
  A["Sistema P: percepciones"] --> B["Signos de percepcion"]
  B --> C["Inconciente"]
  C --> D["Preconciencia"]
  D --> E["Conciencia"]
```
...
```mermaid
flowchart LR
  A["Tension interna"] --> C["Vivencia de satisfaccion"]
  B["Objeto que calma"] --> C
  C --> D["Huella de tension"]
  C --> E["Huella de objeto"]
  D <-->|"simultaneidad"| E
  F["Nueva tension"] -->|"deseo: retorno hacia la huella"| E
```
...
```mermaid
flowchart TD
  A["Representacion irreconciliable"] --> B["Conflicto psiquico"]
  B --> C["Defensa"]
  C --> D["Separacion representacion / monto de afecto"]
  D --> E["Representacion fuera del yo: grupo psiquico segundo"]
  D --> F["Monto de afecto busca otro destino"]
  F --> G["Cuerpo -> conversion -> histeria"]
  F --> H["Otra representacion -> falso enlace -> neurosis obsesiva"]
  C --> I["Desestimacion conjunta R + MA -> psicosis / confusion alucinatoria"]
```
...
```mermaid
flowchart LR
  A["Escena II: 8 anos"] -->|"VSPT: pastelero, risa, vestido"| B["Pubertad"]
  B --> C["Escena I: 11/12 anos"]
  C -->|"empleado, risa, vestido"| D["Recuerdo de escena II"]
  D -->|"desprendimiento retroactivo de displacer"| E["Sintoma: no poder ir sola a la tienda"]
```
...
```mermaid
flowchart LR
  A["1. VSPT"] --> B["2. Represion"]
  B --> C["3. Salud / defensa lograda: sintoma primario"]
  C --> D["4. Retorno de lo reprimido"]
  D --> E["Enfermedad propiamente dicha: sintoma secundario"]
```
...
```mermaid
flowchart TD
  A["Yo"] --> B["Defensa / represion"]
  B --> C["Sintoma"]
  C --> D["Resistencia en el analisis"]
  C --> E["Testimonio del fracaso de la defensa"]
```
...
```mermaid
flowchart LR
  A["Icc"] -->|"via progrediente"| B["Prcc"]
  B --> C["Cc"]
  C -.->|"via regrediente, siempre pasando por Prcc"| B
  B -.-> A
```
...
```mermaid
flowchart LR
  A["Necesidad / tension"] --> B["Primera experiencia de satisfaccion"]
  B --> C["Huella de satisfaccion"]
  D["Nueva tension"] --> E["Busqueda de identidad de percepcion"]
  E --> C
  C --> F["Encuentra huella, no objeto pleno"]
  F --> G["Resto: deseo"]
```
...
flowchart TD
  ...
  B --> C["El yo intenta olvidar"]
  C --> D["Fracaso"]
  D --> E["Defensa"]
  E --> F["Separacion representacion / monto de afecto"]
  F --> G["Representacion: segundo grupo psiquico"]
  F --> H["Monto de afecto: debe ligarse a otra representacion"]
  H --> I["Cuerpo: conversion -> histeria"]
  H --> J["Ideas / falso enlace -> neurosis obsesiva"]
```
...
```mermaid
flowchart LR
  A["Vivencia anterior"] -->|"idoneidad determinadora"| C["Sintoma"]
  B["Vivencia posterior"] -->|"fuerza traumatica"| C
  A <-->|"nexo asociativo / simbolico"| B
```
...
```mermaid
flowchart LR
  A["Escena infantil: 8 anos"] -->|"recuerdo inconciente"| C["Pubertad / segundo tiempo"]
  B["Escena posterior: 11 anos, tienda/panaderia"] -->|"risa + vestidos"| C
  C -->|"asociacion extrinseca"| D["Desprendimiento sexual retroactivo"]
  D --> E["Angustia / sintoma"]
```
